...
import { saveAchievementsToSupabase, loadAchievementsFromSupabase } from './supabase.js';
...
export async function loadPlayerStats() {
    try {
        ...
        if (isLoggedIn()) {
            try {
                // 클라우드에서 데이터 불러오기
                const cloudData = await loadAchievementsFromSupabase();
                
                if (cloudData) {
                    // 클라우드 데이터가 있으면 사용
                    playerStats = { ...playerStats, ...cloudData.playerStats };
                    unlockedAchievements = new Set(cloudData.unlockedAchievements);
                    ...
                    console.log('🔄 클라우드에서 도전과제 데이터 로드 완료');
                    return;
                }
            } catch (error) {
        ...
        console.log('플레이어 통계 및 업적 데이터 로컬 로드 완료');
    } catch (error) {
 ...
 */
export async function initializeNewPlayerStats() {
...
export async function savePlayerStats() {
    try {
        ...
        if (isLoggedIn()) {
            try {
                await saveAchievementsToSupabase(playerStats, [...unlockedAchievements]);
            } catch (error) {
 ...
 */
export async function endGameSession(wave, monstersAvoided, coinsEarned) {
 ...
 */
export async function recordSkillUse(skillType) {
 ...
 * 업그레이드 구매 통계 업데이트
 */
export async function recordUpgradePurchase() {
 ...
 * 뽑기 통계 업데이트
 */
export async function recordGachaPull() {
 ...
 * 도전과제 시스템 초기화
 */
export async function initAchievementSystem() {
